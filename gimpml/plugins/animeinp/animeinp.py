...
model_dict = {MODEL1: "getchu", MODEL2: "celeba", MODEL3: "psv"}
...
def run(procedure, run_mode, image, n_drawables, layer, args, data):
    ...
    if run_mode == Gimp.RunMode.INTERACTIVE:
        ...
        python_path = config_path_output["python_path"]
        config_path_output["plugin_path"] = os.path.join(config_path, PLUGIN_ID, "animeinp.py")

        config = procedure.create_config()
        config.set_property("force_cpu", force_cpu)
        config.begin_run(image, run_mode, args)

        GimpUi.init("animeinp.py")
        ...
        if n_drawables != 1:
            show_dialog(
                "Please select an image layer containing layer mask.", "Error !", "error", image_paths
            )
        ...
        # Create UI
        dialog = GimpUi.Dialog(use_header_bar=use_header_bar, title=_("Anime InPainting..."))
    ...
    def do_create_procedure(self, name):
        ...
        if name == PLUGIN_ID:
            ...
            procedure.set_documentation(
                N_("Performs anime-style inpainting on a given image with GIMP mask layer."),
                globals()["__doc__"], 
                name,
            )
            procedure.set_menu_label(N_("_Anime InPaint..."))
