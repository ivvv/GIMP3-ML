...
def matting(
    ...
    for index, drawable in enumerate(drawables):
        save_image(BASE_IMG.format(str(index)), image, drawable)
    ...
    if data_output["inference_status"] == "success":
        load_single_result_and_insert(RESULT_IMG, image, "Matted")
...
def run(procedure, run_mode, image, n_drawables, layer, args, data):
    ...
    if run_mode == Gimp.RunMode.INTERACTIVE:
        ...
        python_path = config_path_output["python_path"]
        config_path_output["plugin_path"] = os.path.join(config_path, PLUGIN_ID, "matting.py")
        ...
        if n_drawables > 2:
            show_dialog(
                N_("Please select only image and mask layers."), "Error!", "error",
                image_paths
            )
        ...
        dialog.add_button(_("_Do matting"), Gtk.ResponseType.OK)
    ...
    def do_create_procedure(self, name):
        ...
        if name == PLUGIN_ID:
            ...
            procedure.set_documentation(
                N_("Performs image matting using a separate mask layer.\nTrimap layer: use RGB as #808080 (gray) for boundaries,\n#FFFFFF (white) for object and #000000 (black) for background.\nBoth layers should be selected."),
