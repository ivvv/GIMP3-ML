...
Infers the monocular depth of the current layer.
"""
...
def monodepth(procedure, image, drawables, force_cpu, progress_bar, config_path_output):
    ...
    if data_output["inference_status"] == "success":
        load_single_result_and_insert(RESULT_IMG, image, "Monodepth Layer")
...
def run(procedure, run_mode, image, n_drawables, layer, args, data):
    ...
    if run_mode == Gimp.RunMode.INTERACTIVE:
        ...
        python_path = config_path_output["python_path"]
        config_path_output["plugin_path"] = os.path.join(config_path, "semseg", "monodepth.py")
        ...
        dialog = GimpUi.Dialog(use_header_bar=use_header_bar, title=_("Monodepth Inferrence..."))
        dialog.add_button(_("_Cancel"), Gtk.ResponseType.CANCEL)
        dialog.add_button(_("_Help"), Gtk.ResponseType.APPLY)
        dialog.add_button(_("_Infer depth"), Gtk.ResponseType.OK)
    ...
    def do_create_procedure(self, name):
        ...
        if name == PLUGIN_ID:
            ...
            procedure.set_documentation(
                N_("Infers the monocular depth of the current layer."),
...
Gimp.main(DepthInfer.__gtype__, sys.argv)
