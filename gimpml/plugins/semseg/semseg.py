...
def semseg(procedure, image, drawables, force_cpu, progress_bar, config_path_output):
    ...
    save_image(os.path.join(tmp_path, BASE_IMG), image, drawables[0])
    ...
    if data_output["inference_status"] == "success":
        gimp_layer = load_image_as_layer(os.path.join(tmp_path, RESULT_IMG), image)
        if (gimp_layer.index(0) == Gimp.PDBStatusType.SUCCESS):
            gimp_layer = gimp_layer.index(1)
            gimp_layer.set_name("Semantic Segmentation")
            gimp_layer.set_mode(Gimp.LayerMode.NORMAL_LEGACY)
            gimp_layer.set_opacity(50)
...
def run(procedure, run_mode, image, n_drawables, layer, args, data):
    ...
    if run_mode == Gimp.RunMode.INTERACTIVE:
        ...
        python_path = config_path_output["python_path"]
        config_path_output["plugin_path"] = os.path.join(config_path, PLUGIN_ID, "semseg.py")
        ...
        dialog.add_button(_("_Infer"), Gtk.ResponseType.OK)
