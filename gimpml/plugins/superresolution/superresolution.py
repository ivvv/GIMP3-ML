...
Performs super-resolution on the current layer.
"""
import gi
...
def super_resolution(
    ...
    if data_output["inference_status"] == "success":
        if scale == 1:
            load_single_result_and_insert(RESULT_IMG, image, "Super-Resolution")
        else:
            image_new = Gimp.Image.new(
                drawables[0].get_width() * scale, drawables[0].get_height() * scale, 0
            )  # 0 for RGB
            display = Gimp.Display.new(image_new)
            gimp_layer = load_image_as_layer(os.path.join(tmp_path, RESULT_IMG), image_new)
            if (gimp_layer.index(0) == Gimp.PDBStatusType.SUCCESS):
                gimp_layer = gimp_layer.index(1)
                gimp_layer.set_name(N_("Super-Resolution"))
                gimp_layer.set_mode(Gimp.LayerMode.NORMAL_LEGACY)
                image_new.insert_layer(gimp_layer, None, -1)
...
def run(procedure, run_mode, image, n_drawables, layer, args, data):
    ...
    if run_mode == Gimp.RunMode.INTERACTIVE:
        ...
        config_path_output["plugin_path"] = os.path.join(
            config_path, PLUGIN_ID, "superresolution.py"
        )
        ...
        dialog.add_button(_("_Upscale"), Gtk.ResponseType.OK)
        ...
        while True:
            ...
            if response == Gtk.ResponseType.OK:
                ...
                filtered = config.get_property("filter")
    ...
    def do_create_procedure(self, name):
        ...
        if name == PLUGIN_ID:
            ...
            procedure.set_documentation(
                N_("Performs super-resolution on the currently selected layer."),
