...
Detects objects on the current layer and filters images correspondingly.
"""
import gi
...
def filterfolder(procedure, args_dict, config_path_output):
    ...
    set_model_config(args_dict, PLUGIN_ID)
    ...
    if data_output["inference_status"] == "success":
        ...
        show_dialog(
            "Filtering was incomplete due to errors.\nYou can check log in " + elog + "...txt",
            "Error!", "error",
            image_paths
        )
        return procedure.new_return_values(Gimp.PDBStatusType.SUCCESS, GLib.Error())
