import os
import pickle
from gimpml.plugins.module_utils import get_weight_path, tmp_path
import subprocess
import numpy as np
import cv2
...
def run(procedure, **kwargs):
    ...
    if "save_path" not in kwargs.keys():
        kwargs["save_path"] = os.path.join(tmp_path, "tmp_filter2.png")
        return_image = True
    if "image" in kwargs.keys():
        image_path = os.path.join(tmp_path, "tmp_filter.png")
